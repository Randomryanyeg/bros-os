...
export const generateStatementHTML = (accountName: string, account: ScotiaAccount, month: number, year: number) => {
  ...
  const employment = ((config.scotia_config as Record<string, unknown>).employment || {
      employer: "GOVERNMENT OF ALBERTA",
      job_title: "Analyst",
      annual_income: 85000
  }) as Record<string, unknown>;
  
  const employerName = String(employment.employer || "GOVERNMENT OF ALBERTA").toUpperCase();
  const netBiMonthly = ((Number(employment.annual_income) || 85000) / 12 / 2) * 0.76;
